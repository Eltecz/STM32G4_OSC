/*
 * @Description: 
 * @FilePath: \MDK-ARM\usr\sys.c
 * @Version: 2.0
 * @Autor: Eltecz
 * @Date: 2022-08-21 22:53:40
 * @LastEditors: Eltecz
 * @LastEditTime: 2022-10-26 22:55:41
 */
#include "per_conf.h"
...
//返回值:0,成功;1,失败。
void Sys_Clock_Set(u32 plln,u32 pllm,u32 pllp,u32 pllr)
{ 
	u8 status;
//	PWR->CR3&=~(1<<2);				//SCUEN=0,锁定LDOEN和BYPASS位的设置
	...
	RCC->PLLCFGR|=(pllp-1)<<27;
//	RCC->PLLCFGR|=(pllq-1)<<21;
	RCC->PLLCFGR|=(pllr-1)<<25;
	RCC->PLLCFGR|=1<<16;	
	RCC->PLLCFGR|=1<<20;
	RCC->PLLCFGR|=1<<24;
	RCC->PLLCFGR&=~(3<<0);		//清零
	...
	while(!((RCC->CR)&&(1<<25)));//PLL1RDY=1?,等待PLL1准备好  	
	
	FLASH->ACR|=5<<0;			//LATENCY[2:0]=2,2个CPU等待周期(@VOS1 Level,maxclock=210Mhz)
	FLASH->ACR|=1<<8;
	
	FLASH->ACR|=1<<9;			//d-cache enable
	FLASH->ACR|=1<<10;			//i-cache enable
...
{  
	...
	Sys_Clock_Set(plln/2,pllm,pllp,pllr);//设置时钟  
...
#endif 
	RCC->APB2ENR|=1<<0;			//启动SYSCONFIG,COMP,VREFBUF,OPAMP
