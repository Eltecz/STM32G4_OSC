...
u8 exfuns_init(void)
{
	...
	{
		fs[i]=(FATFS*)mymalloc(SRAM1,sizeof(FATFS));	//为磁盘i工作区申请内存	
		if(!fs[i])break;
	}
	file=(FIL*)mymalloc(SRAM1,sizeof(FIL));		//为file申请内存
	ftemp=(FIL*)mymalloc(SRAM1,sizeof(FIL));		//为ftemp申请内存
	fatbuf=(u8*)mymalloc(SRAM1,512);				//为fatbuf申请内存
...
u8 exf_copy(u8(*fcpymsg)(u8*pname,u8 pct,u8 mode),u8 *psrc,u8 *pdst,u32 totsize,u32 cpdsize,u8 fwmode)
{
	...
	unsigned long long lcpdsize=cpdsize; 
 	fsrc=(FIL*)mymalloc(SRAM1,sizeof(FIL));//申请内存
 	fdst=(FIL*)mymalloc(SRAM1,sizeof(FIL));
	fbuf=(u8*)mymalloc(SRAM1,8192);
...
#define MAX_PATHNAME_DEPTH	512+1	//最大目标文件路径+文件名深度
	...
	fddir=(DIR*)mymalloc(SRAM1,sizeof(DIR));//申请内存
 	finfo=(FILINFO*)mymalloc(SRAM1,sizeof(FILINFO));
   	if(fddir==NULL||finfo==NULL)res=100;
	if(res==0)
	{ 
 		pathname=mymalloc(SRAM1,MAX_PATHNAME_DEPTH);	    
...
#define MAX_PATHNAME_DEPTH	512+1	//最大目标文件路径+文件名深度
	...
	srcdir=(DIR*)mymalloc(SRAM1,sizeof(DIR));//申请内存
 	dstdir=(DIR*)mymalloc(SRAM1,sizeof(DIR));
	finfo=(FILINFO*)mymalloc(SRAM1,sizeof(FILINFO));

   	if(srcdir==NULL||dstdir==NULL||finfo==NULL)res=100;
	if(res==0)
	{ 
 		dstpathname=mymalloc(SRAM1,MAX_PATHNAME_DEPTH);
		srcpathname=mymalloc(SRAM1,MAX_PATHNAME_DEPTH);
 		...
 		if(res==0)
		{
		    ...
		    if(res==0)//打开目录成功 
			{
  			...
  			myfree(SRAM1,dstpathname);
 			myfree(SRAM1,srcpathname); 
		}
 	}
	myfree(SRAM1,srcdir);
	myfree(SRAM1,dstdir);
