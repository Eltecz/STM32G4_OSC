...
{  
	...
	W25QXX_CS(0);                            //使能器件   
	SPI3_WriteByte(command);            //发送读取状态寄存器命令    
...
{   
	...
	W25QXX_CS(0);                            //使能器件   
	SPI3_WriteByte(command);            //发送写取状态寄存器命令    
	SPI3_WriteByte(sr);                 //写入一个字节  
	W25QXX_CS(1);                            //取消片选     	      
}   
//W25QXX写使能	
//将WEL置位   
...
{
	W25QXX_CS(0);                            //使能器件   
    SPI3_WriteByte(W25X_WriteEnable);   //发送写使能  
	W25QXX_CS(1);                            //取消片选     	      
} 
//W25QXX写禁止	
//将WEL清零  
...
{  
	W25QXX_CS(0);                            //使能器件   
    SPI3_WriteByte(W25X_WriteDisable);  //发送写禁止指令    
	W25QXX_CS(1);                            //取消片选     	      
} 
...
{ 
 	u16 i;   										    
	W25QXX_CS(0);                           //使能器件   
    SPI3_WriteByte(W25X_FastReadData);	//发送快速读取命令（SPI CLK可以高达104Mhz）  
    if(W25QXX_TYPE==W25Q256)                //如果是W25Q256的话地址为4字节的，要发送最高8位
    {
        SPI3_WriteByte((u8)((ReadAddr)>>24));    
    }
    SPI3_WriteByte((u8)((ReadAddr)>>16));   //发送24bit地址    
    SPI3_WriteByte((u8)((ReadAddr)>>8));   
    SPI3_WriteByte((u8)ReadAddr);   
	SPI3_WriteByte(0XFF);
    for(i=0;i<NumByteToRead;i++)
	{ 
        pBuffer[i]=SPI3_ReadByte(0XFF);    //循环读数  
    }
	W25QXX_CS(1);  				    	      
}  
...
void W25QXX_Write_Page(u8* pBuffer,u32 WriteAddr,u16 NumByteToWrite)
{
	...
	W25QXX_CS(0);                            //使能器件   
    SPI3_WriteByte(W25X_PageProgram);   //发送写页命令   
    if(W25QXX_TYPE==W25Q256)                //如果是W25Q256的话地址为4字节的，要发送最高8位
    {
        SPI3_WriteByte((u8)((WriteAddr)>>24)); 
    }
    SPI3_WriteByte((u8)((WriteAddr)>>16)); //发送24bit地址    
    SPI3_WriteByte((u8)((WriteAddr)>>8));   
    SPI3_WriteByte((u8)WriteAddr);   
    for(i=0;i<NumByteToWrite;i++)SPI3_WriteByte(pBuffer[i]);//循环写数  
	W25QXX_CS(1);                            //取消片选 
...
{                                   
    ...
    W25QXX_Wait_Busy();   
  	W25QXX_CS(0);                           //使能器件   
    SPI3_WriteByte(W25X_ChipErase);		//发送片擦除命令  
	W25QXX_CS(1);							//取消片选     	      
...
{  
    ...
    W25QXX_Wait_Busy();   
  	W25QXX_CS(0);							//使能器件   
    SPI3_WriteByte(W25X_SectorErase);   //发送扇区擦除指令 
    if(W25QXX_TYPE==W25Q256)                //如果是W25Q256的话地址为4字节的，要发送最高8位
    {
        SPI3_WriteByte((u8)((Dst_Addr)>>24)); 
    }
    SPI3_WriteByte((u8)((Dst_Addr)>>16));  //发送24bit地址    
...
{ 
  	W25QXX_CS(0);                           //使能器件   
    SPI3_WriteByte(W25X_PowerDown);     //发送掉电命令  
	W25QXX_CS(1);                           //取消片选     	      
    delay_us(3);                            //等待TPD  
}   
//唤醒
